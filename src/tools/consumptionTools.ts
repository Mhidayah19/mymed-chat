...
export const getConsumptionRequest = tool({
  description:
    "Retrieve detailed information about a specific consumption request",
  parameters: z.object({
    id: z.string().describe("The ID of the consumption request to retrieve"),
    expand: z
      .array(z.string())
      .optional()
      .describe("Optional associations to expand in the response"),
  }),
...
export const getConsumptionRequests = tool({
  description:
    "Retrieve multiple consumption requests with optional filtering and pagination",
  parameters: z.object({
    limit: z
      .number()
      .optional()
      .describe("Maximum number of requests to retrieve"),
    skip: z
      .number()
      .optional()
    ...
    orderBy: z
      ...
      .optional()
      .describe(
        "Field(s) to order results by example: 'createdAt' (not 'createdDate')"
      ),
    filter: z
      .record(z.any())
      .optional()
      .describe("Filter criteria for the requests"),
    expand: z
      .array(z.string())
      .optional()
      .describe("Associations to expand in the response"),
    createdBy: z
      .string()
      .optional()
      .describe("Filter by the user who created the request"),
  }),
  execute: async ({
    limit = 10,
    skip = 0,
    ...
    console.log(
      `Fetching consumption requests with limit: ${limit}, skip: ${skip}`
    );
      ...
      if (Object.keys(filter).length > 0) {
        let filterString = Object.entries(filter)
          .map(([key, value]) => {
            ...
            if (
              correctedKey === "status_code" ||
              correctedKey === "customer_ID"
            ) {
...
export const getConsumptionRequestsByStatus = tool({
  ...
  parameters: z.object({
    statusCode: z
      .string()
      .describe(
        "Status code to filter by (e.g., 'O', 'P', 'W', 'R', 'T', 'I', 'E')"
      ),
    limit: z
      .number()
      .optional()
...
export const getConsumptionRequestsByCustomer = tool({
  ...
  execute: async ({ customerId, limit = 10, skip = 0, expand = [] }) => {
    ...
    try {
      ...
      console.error(
        `Error fetching consumption requests for customer: ${error}`
      );
...
export const searchConsumptionRequestsByCaseRef = tool({
  ...
  execute: async ({ caseRef, limit = 10, skip = 0, expand = [] }) => {
    ...
    try {
      ...
      console.error(
        `Error searching consumption requests by case reference: ${error}`
      );
...
export const getConsumptionRequestStatusCodes = tool({
  description:
    "Retrieve all available consumption request status codes and their descriptions",
  parameters: z.object({}),
  execute: async () => {
    ...
    try {
      ...
      const statusCodes = await fetchApi(
        `catalog/ConsumptionService/ConsumptionRequestStatus`
      );

      return statusCodes;
    } catch (error) {
      console.error(
        `Error fetching consumption request status codes: ${error}`
      );
