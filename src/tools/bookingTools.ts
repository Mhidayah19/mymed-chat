...
export const getBookingInformation = tool({
  ...
  execute: async ({ bookingId, isActiveEntity }) => {
    console.log(
      `Fetching booking information for booking ID: ${bookingId}, isActiveEntity: ${isActiveEntity}`
    );
    ...
    try {
      ...
      const bookingInfo = await fetchApi(
        `catalog/BookingService/Bookings(ID=${bookingId},IsActiveEntity=${isActiveEntity})?${queryParams.toString()}`
      );
...
export const getAllBookings = tool({
  ...
  execute: async ({ limit = 10, offset = 0, status = "" }) => {
    console.log(
      `Fetching all bookings with limit: ${limit}, offset: ${offset}, status: ${status}`
    );
    ...
    try {
      ...
      const bookings = await fetchApiWithParams(
        "catalog/BookingService/Bookings",
        params
      );
...
export const getBookingTypes = tool({
  description:
    "Retrieve all booking types and their descriptions, helpful for the user to select the correct booking type",
  parameters: z.object({}),
  execute: async () => {
    ...
    try {
      ...
      const bookingTypes = await fetchApi(
        "catalog/BookingService/BookingTypes"
      );
...
export const getCustomers = tool({
  ...
  execute: async ({ limit = 10, offset = 0, search = "" }) => {
    console.log(
      `Fetching customers with limit: ${limit}, offset: ${offset}, search: ${search}`
    );
    ...
    try {
      ...
      const customers = await fetchApiWithParams(
        "catalog/BookingService/Customers",
        params
      );
...
export const getShipToAddresses = tool({
  description:
    "Retrieve a list of shipping addresses (shipTo) that can be used for bookings",
  parameters: z.object({
    limit: z
      .number()
      .optional()
  ...
  execute: async ({ limit = 10, offset = 0, search = "" }) => {
    console.log(
      `Fetching shipping addresses with limit: ${limit}, offset: ${offset}, search: ${search}`
    );
    ...
    try {
      ...
      const shipToAddresses = await fetchApiWithParams(
        "catalog/BookingService/BusinessPartners",
        params
      );
...
export const getMaterials = tool({
  description:
    "Retrieve a list of available materials that can be added to bookings",
  parameters: z.object({
    limit: z
      .number()
      .optional()
  ...
  execute: async ({ limit = 10, offset = 0, search = "", category = "" }) => {
    console.log(
      `Fetching materials with limit: ${limit}, offset: ${offset}, search: ${search}, category: ${category}`
    );
    ...
    try {
      ...
      const materials = await fetchApiWithParams(
        "catalog/BookingService/Materials",
        params
      );
...
export const bookingExecutions = {
  createBooking: async ({
    ...
    console.log(
      `Creating booking for customer ID: ${soldTo_ID}, ship to ID: ${shipTo_ID}, type: ${bookingType}, delivery date: ${requestedDeliveryDate}`
    );
    ...
    try {
      ...
      // Use fetchApi for the API call
      const newBooking = (await fetchApi("catalog/BookingService/Bookings", {
        method: "POST",
        headers: {
  ...
  updateBooking: async ({
    ...
    console.log(
      `Updating booking ID: ${bookingId}, isActiveEntity: ${isActiveEntity}`
    );

    try {
      // First, get the current booking to update only changed fields
      const currentBooking = await fetchApi(
        `catalog/BookingService/Bookings(ID=${bookingId},IsActiveEntity=${isActiveEntity})`
      );
