...
export const ChatMaterialCard = ({
  material,
}: {
  material: ChatMaterialInfo;
}) => {
  ...
  const handleCardClick = () => {
    if (material.id) {
      const baseUrl =
        "https://mymediset-xba-dev-eu10.launchpad.cfapps.eu10.hana.ondemand.com/site?siteId=04bd86f5-c383-41a9-966a-c97d7744a8ea#cloudmymedisetuimaterials-manage?sap-ui-app-id-hint=mymediset_cloud.mymediset.uimaterials&/Materials";
    ...
    >
      <div className="p-3">
        ...
        <div className="grid grid-cols-1 gap-1 text-xs text-neutral-600 dark:text-neutral-300">
          {material.quantity && (
            <div
              className={`flex items-center fade-in ${animationStage >= 2 ? "" : "opacity-0"} delay-200`}
            >
          ...
          {material.category && (
            <div
              className={`flex items-center fade-in ${animationStage >= 3 ? "" : "opacity-0"} delay-300`}
            >
              <span className="font-medium mr-2">Category:</span>
              <span>
                {material.category}
                {animationStage === 3 && showTypingCursor && (
                  <span className="typing-cursor">&nbsp;</span>
                )}
              </span>
            </div>
          ...
          {material.expiryDate && (
            <div
              className={`flex items-center fade-in ${animationStage >= 4 ? "" : "opacity-0"} delay-400`}
            >
              <span className="font-medium mr-2">Expiry Date:</span>
              <span>
                {material.expiryDate}
                {animationStage === 4 && showTypingCursor && (
                  <span className="typing-cursor">&nbsp;</span>
                )}
              </span>
            </div>
          ...
          {material.id && (
            <div
              className={`flex items-center fade-in ${animationStage >= 5 ? "" : "opacity-0"} delay-500`}
            >
              <span className="font-medium mr-2">Material ID:</span>
              <span className="font-mono">
                {material.id}
                {animationStage === 5 && showTypingCursor && (
                  <span className="typing-cursor">&nbsp;</span>
                )}
              </span>
            </div>
...
const extractMaterialDetails = (content: string): ChatMaterialInfo => {
  ...
  lines.forEach((line) => {
    if (line.startsWith("name:")) material.name = line.slice(5).trim();
      ...
      material.quantity = line.slice(9).trim();
    else if (line.startsWith("category:"))
      material.category = line.slice(9).trim();
    else if (line.startsWith("expiryDate:"))
      material.expiryDate = line.slice(11).trim();
    else if (line.startsWith("status:"))
      material.status = line.slice(7).trim().toLowerCase();
