  ...
  return (
    <Card
      className={`p-4 my-3 w-full max-w-[500px] rounded-md bg-neutral-100 dark:bg-neutral-900 ${
        needsConfirmation ? "" : "border-[#2F366D]/30"
      } overflow-hidden`}
    >
      <button
        type="button"
      ...
      >
        <div
          className={`${needsConfirmation ? "bg-[#2F366D]/10" : "bg-[#2F366D]/5"} p-1.5 rounded-full flex-shrink-0`}
        >
          <Robot size={16} className="text-[#2F366D]" />
        </div>
        <h4 className="font-medium flex items-center gap-2 flex-1 text-left">
          {toolInvocation.toolName}
          {!needsConfirmation && toolInvocation.state === "result" && (
            <span className="text-xs text-[#2F366D]/70">✓ Completed</span>
          )}
        </h4>
        <CaretDown
          size={16}
